...
public class ChuancCiExecuteSingleActivity extends BaseScanTestTempleActivity {
...
//    private UnPutInfusionDetailDAO unPutInfusionDetailDAO;
    ...
    public void setContentView() {
        setContentView(R.layout.activity_excute_single);
    }


    @Override
    public void initView() {
        printModeString = XmlDB.getInstance(ChuancCiExecuteSingleActivity.this).getKeyString("printSetting","蓝牙打印");
        isUseSeatNo = XmlDB.getInstance(context).getKeyBooleanValue("isUseSeatNo", false);
        strings = ResourceUtils.getResouce4Arrays(ChuancCiExecuteSingleActivity.this, R.array.infusionstatus_handexecute_chuanci);
        infusionDetailDAO = new InfusionDetailDAO(DataBaseInfo.getInstance(ChuancCiExecuteSingleActivity.this));
        InitCombox();
        ...
        if (RoleCategory.CHUANCI.getKey() == LocalSetting.RoleIndex) {
            if(isUseSeatNo){
                ...
                if(printModeString.contains("蓝牙打印")){
                    InfoUtils.checkBlueTooth(ChuancCiExecuteSingleActivity.this);
                }
...
//            btnCancel.setVisibility(QueueStatusCategory.FINISHED.getKey() == LocalSetting.CurrentBottle.PeopleInfo.Status ? View.GONE : View.VISIBLE);
    ...
    public void initData() {
        loadingDialog = new ProgressDialog(ChuancCiExecuteSingleActivity.this);
        beepManager = new BeepManager(ChuancCiExecuteSingleActivity.this);
        punctureDeptId = XmlDB.getInstance(context).getKeyIntValue("punctureDeptId", 15) + "";
        infusionDetailDAO = new InfusionDetailDAO(DataBaseInfo.getInstance(ChuancCiExecuteSingleActivity.this));
//        unPutInfusionDetailDAO = new UnPutInfusionDetailDAO(DataBaseInfo.getInstance(ChuancCiExecuteSingleActivity.this));
        uploadInfusionDetailDAO = new UploadInfusionDetailDAO(DataBaseInfo.getInstance(ChuancCiExecuteSingleActivity.this));
        ...
        btnOK.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if(StringUtils.getString(currentBottle.DiagnoseName).equals("1")){
                    Intent intent=new Intent(ChuancCiExecuteSingleActivity.this, CheckActivity.class);
...
//                excuteAfter(true);
                ChuancCiExecuteSingleActivity.this.finish();
            }
     ...
     */
    private void excute() {
      /*  if(currentBottle.BottleStatus == BottleStatusCategory.INFUSIONG.getKey()){
            unPutInfusionDetailDAO.deleteBottleByQueueNo(currentBottle.BottleId);
        }*/
        ...
        for (BottleModel each : oldBottles) {
            ...
            each.PeopleInfo.Status = QueueStatusCategory.FINISHED.getKey();
        ...
        if (uploadInfusionDetailDAO.getBottleById(currentBottle.BottleId).BottleId != null) {
            uploadInfusionDetailDAO.updateBottle(currentBottle);
        } else {
            uploadInfusionDetailDAO.saveExecuteSingBottleModel(currentBottle);
        }
            ...
            if (!StringUtils.StringIsEmpty(currentBottle.PeopleInfo.SeatNo)) {
                if(isUseSeatNo){
                    if(printModeString.contains("蓝牙打印")){
                        ...
                        InfoUtils.bluetoothPrintSeatNo(context, divZone+":"+currentBottle.PeopleInfo.SeatNo, PeopleContent);
                    }
                }
            } else {
...
//                        doCallHer();
                        ...
                        UIHepler.showToast(ChuancCiExecuteSingleActivity.this, "重新穿刺病人"); //不呼叫
                    }
                }
            }
    ...
    private void doPrintSeatNo() {
        ...
        if (!StringUtils.getString(currentBottle.GCF).equals("重症")) {
            InfoUtils.checkBlueTooth(ChuancCiExecuteSingleActivity.this);
            if (StringUtils.StringIsEmpty(currentBottle.PeopleInfo.SeatNo)) {
                UIHepler.showToast(context, "还未分配座位，无法打印");
                return;
            }
            if(StringUtils.StringIsEmpty(divZone)){
            ...
            String PeopleContent=currentBottle.PeopleInfo.Name +":" +currentBottle.PeopleInfo.PatId ;
            InfoUtils.bluetoothPrintSeatNo(context, StringUtils.getString(divZone) + ":" + currentBottle.PeopleInfo.SeatNo,PeopleContent);
            //        InfoUtils.bluetoothPrintSeatNo(context, "加号输液区"+":"+"+01");
            //+"\t\t"+StringUtils.getString(FQ));

        } else {
            UIHepler.showToast(context, "重症病人，无法打印座位号");
        }
...
//                                                            patrolDetailBean.Expand = inputText;
                                            ...
                                            btnOK.setEnabled(cbBottle.isChecked() && cbBand.isChecked());
//                                            btnOK.setEnabled(true);
                                            WarningToast.newInstance(ChuancCiExecuteSingleActivity.this, "核对病人信息成功");

                                        } else {
                                            cbBand.setChecked(false);
                                            InfusionUIHelper.showWarningDialogByCustom(context, "病人ID录入错误!");
                                            InfoUtils.warningComfirmError(ChuancCiExecuteSingleActivity.this,beepManager);
                                            ...
                                            InfoUtils.recordConfirmError(ChuancCiExecuteSingleActivity.this,infusionEventModels);
...
//                                        btnOK.setEnabled(true);
    ...
    private void doAllotSeat() {
        ...
        if (LocalSetting.AutoAllotSeat) {
            ...
            intent.setClass(ChuancCiExecuteSingleActivity.this, AreaInfusionEmptyActivity.class);
...
//        }
        ...
        if (QueueStatusCategory.WAITING.getKey() == currentBottle.PeopleInfo.Status) {
            ...
            RestClient.put(context, ChuanCiApi.url_updateQueues(deptID, punctureDeptId), jsonString, new BaseAsyncHttpResponseHandler() {
                @Override
                public void onSuccess(int i, String s) {
                    dismissLoadingDialog();
                    if (!StringUtils.getString(currentBottle.GCF).equals("重症")) {
             ...
             /* divZone=Fq[1];
              LocalSetting.divZone=Fq[1];*/
                ...
                @Override
                public void onFailure(int i, Throwable throwable, String s) {
                    UIHepler.showToast(ChuancCiExecuteSingleActivity.this,"呼叫失败");
                    dismissLoadingDialog();
                }
            });
...
//                        UIHepler.showToast(context, name + ": [" + changeQueueNo + "]" + "呼叫成功，分配了" + Fq[0] + "座位!");
                        ...
                        if(isUseSeatNo){
                            ...
                            if(string.equals("ekyy")){
                                ...
                                UIHepler.showToast(context, name + ": [" + changeQueueNo + "]" + "呼叫成功");
                            }
                            btnSeat.setVisibility(View.GONE);
                        }
                        currentBottle.PeopleInfo.SeatNo = Fq[0].replace("号", " ").trim();//用于蓝牙打印座位，
            ...
            UIHepler.showToast(ChuancCiExecuteSingleActivity.this,"当前病人已完成穿刺,无需再进行呼叫");
        }
    }
    /**ggf
     * 手动执行呼叫
...
//        btnOK.setEnabled(true);
            ...
            RestClient.get(url, new BaseAsyncHttpResponseHandler() {
                ...
                public void onSuccess(int i, String s) {
                    ...
                    if (bottleModel != null) {
                        ...
                        UIHepler.showToast(ChuancCiExecuteSingleActivity.this, "没有查找带该瓶贴！");
                    }
                ...
                public void onFailure(int i, Throwable throwable, String s) {
                    dismissLoadingDialog();
                    UIHepler.showToast(ChuancCiExecuteSingleActivity.this, "查找失败");
                }
            });
        }
    }
    ...
    protected void showPopMenu() {
        if (popMenu == null) {
            popMenu = new PopMenu(ChuancCiExecuteSingleActivity.this);
    ...
    private void UpOrNextBottle(String str){
        ...
        if("up".equals(str)){
            if(loadIndex>0){
                ...
                UIHepler.showToast(ChuancCiExecuteSingleActivity.this,"没有上一个了");
            }
        }else if("next".equals(str)){
            if(loadIndex<bottleModels.size()-1){
                loadBottle2(bottleModels.get(loadIndex+1));
            }else {
                UIHepler.showToast(ChuancCiExecuteSingleActivity.this,"没有下一个了");
            }
        }
    }
...
//          ListViewUtils.setListViewHeightBasedOnChildren(listView);
    ...
    public void dismissLoadingDialog() {
        if(this!=null && !this.isFinishing() && loadingDialog !=null && loadingDialog.isShowing()){
