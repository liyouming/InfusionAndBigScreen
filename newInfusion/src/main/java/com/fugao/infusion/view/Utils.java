...
import com.fugao.infusion.utils.FileHelper;
...
class Utils {
  ...
  static boolean isEnabled(int[] stateSet) {
    for (int state : stateSet)
      if (state == R.attr.state_enabled) return true;
    return false;
  }
  ...
  static File resourceToFile(Context context, String resourceName) throws IOException {
    ...
    try {
      ...
      inputStream = FileHelper.getInputStreamFromAssets("font/" + resourceName, context);
