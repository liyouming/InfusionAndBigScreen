...
public class XHHandlerActivity extends BaseTempleActivity
...
//    private InfusionDetailDAO infusionDetailDAO;
    ...
    public void setContentView() {
        setContentView(R.layout.activity_handler_xh);
    }
    ...
    public void initView() {
        ...
        printModeString = XmlDB.getInstance(XHHandlerActivity.this).getKeyString("printSetting","蓝牙打印");
        if(isUseSeatNo){
            if(printModeString.contains("蓝牙打印")){
                InfoUtils.checkBlueTooth(XHHandlerActivity.this);
            }
        }
        if (isHide) {
        ...
        String string = PullXMLTools.parseXml(XHHandlerActivity.this, Constant.DEFAUL_CONFIG_XML, "hospital");
        if(string.equals("zzey")){
            btnAllotSeat.setVisibility(View.INVISIBLE);
        }else if(string.equals("xhyy")) {
            btnAllotSeat.setVisibility(View.INVISIBLE);
        }else{
            btnAllotSeat.setVisibility(View.VISIBLE);
        }
    }

    @Override
    public void initData() {
        createProgressDialog(XHHandlerActivity.this);
        initProcessButtonList();
//        infusionDetailDAO = new InfusionDetailDAO(DataBaseInfo.getInstance(XHHandlerActivity.this));
    ...
    private void doArround() {
        ...
        RestClient.put(XHHandlerActivity.this, url, Json, new BaseAsyncHttpResponseHandler() {
            @Override
            public void onSuccess(int i, String s) {
                dismissLoadingDialog();
                realeaseAllButtons();
                UIHepler.showToast(XHHandlerActivity.this, "添加巡视记录成功");
                openActivity(XHXunshiActivity.class);
                finish();
    ...
    private void doException() {
        ...
        if (otherReason == null) {
            UIHepler.showToast(XHHandlerActivity.this, "请选择异常巡视现象");
            return;
        }
        if (aboutPatrols != null) {
            aboutPatrols.add(patrolDetailBean);
            ...
            RestClient.get(url, new BaseAsyncHttpResponseHandler() {
                ...
                public void onSuccess(int i, String s) {
                    ...
                    if(seatMessage.contains("none")){
                        UIHelper.showToast(XHHandlerActivity.this,"没有剩余座位,请稍后重试!");
                    } else if(seatMessage.contains("has")){
                        UIHelper.showToast(XHHandlerActivity.this,"该座位已经被占用,请重新分配");
                    }else {
                        /**
                         * 更新数据库
                         */
...
//                        infusionDetailDAO.updateBottle(currentBottle);
                        ...
                        InfoUtils.bluetoothPrintSeatNo(XHHandlerActivity.this,seatMessage,PeopleContent);
                ...
                public void onFailure(int i, Throwable throwable, String s) {
                    dismissLoadingDialog();
                    realeaseAllButtons();
                    UIHelper.showToast(XHHandlerActivity.this,"自动分配座位失败");
                }
            });
        } else {
            //            /**
            ...
            //             */
            //            doShowEmptySeats();
            startActivityForResult(new Intent(XHHandlerActivity.this,AreaInfusionEmptyActivity.class), 100);
        }
    }
    /**
     * 结束本次输液
    ...
    private void doFinish() {   //先判断有多少未输液
        ...
        RestClient.get(url, new BaseAsyncHttpResponseHandler() {
            ...
            @Override
            public void onFailure(int i, Throwable throwable, String s) {
                UIHepler.showToast(XHHandlerActivity.this, "判断几组未输失败");
            }
        });
    }
...
//                    int unBottlesCount = infusionDetailDAO.getUnBottlesCount(currentBottle.PeopleInfo.PatId);
                    UIHepler.showToast(XHHandlerActivity.this, "结束本次输液成功");
                    finish();
//                    if (unBottlesCount > 0) {
//                        AlertDialog.Builder dialog = new AlertDialog.Builder(XHHandlerActivity.this);
...
//                    }
            ...
            public void onFailure(int i, Throwable throwable, String s) {
                ...
                UIHepler.showToast(XHHandlerActivity.this, "结束本次输液失败");
            }
        });
    ...
    private void handlerGameOver() {
        ...
        RestClient.get(url, new BaseAsyncHttpResponseHandler() {
            ...
            public void onSuccess(int i, String s) {
                ...
                InfoUtils.sendSuccessBroadcast(context);
                UIHepler.showToast(XHHandlerActivity.this, "结束全部输液成功");
                finish();
            }
            ...
            public void onFailure(int i, Throwable throwable, String s) {
                dismissLoadingDialog();
                realeaseAllButtons();
                UIHepler.showToast(XHHandlerActivity.this, "结束所有输液失败");
            }
        });
    }
        ...
        RestClient.get(url, new BaseAsyncHttpResponseHandler() {
            ...
            public void onSuccess(int i, String s) {
                ...
                if ("none".equals(seatMessage)) {
                    UIHepler.showDilalog(XHHandlerActivity.this,"没有剩余座位,请稍后重试!");
                } else if ("has".equals(seatMessage)) {
                    UIHepler.showDilalog(XHHandlerActivity.this,emptySeatNO1 + "座位被占用，请选择其他座位！");
                } else {
                    ...
                    if(isUseSeatNo){
                        if(printModeString.contains("蓝牙打印")){
                            String PeopleContent=currentBottle.PeopleInfo.Name +":" +currentBottle.PeopleInfo.PatId ;
                            InfoUtils.bluetoothPrintSeatNo(XHHandlerActivity.this,StringUtils.getString(divZone) + ":" + currentBottle.PeopleInfo.SeatNo,PeopleContent);
                        }
                    }
