...
public class NewHandlerActivity extends BaseTempleActivity
    ...
    public void initView() {
        ...
        printModeString = XmlDB.getInstance(NewHandlerActivity.this).getKeyString("printSetting","蓝牙打印");
        if(isUseSeatNo){
            if(printModeString.contains("蓝牙打印")){
                InfoUtils.checkBlueTooth(NewHandlerActivity.this);
            }
        }
        if (isHide) {
        ...
        String string = PullXMLTools.parseXml(NewHandlerActivity.this, Constant.DEFAUL_CONFIG_XML, "hospital");
    ...
    public void initData() {
        createProgressDialog(NewHandlerActivity.this);
        initBoradCast();
        initProcessButtonList();
        infusionDetailDAO = new InfusionDetailDAO(DataBaseInfo.getInstance(NewHandlerActivity.this));
    ...
    private void doArround() {
        ...
        RestClient.put(NewHandlerActivity.this, url, Json, new BaseAsyncHttpResponseHandler() {
            @Override
            public void onSuccess(int i, String s) {
                dismissLoadingDialog();
                realeaseAllButtons();
                UIHepler.showToast(NewHandlerActivity.this, "添加巡视记录成功");
    ...
    private void doException() {
        ...
        if (otherReason == null) {
            UIHepler.showToast(NewHandlerActivity.this, "请选择异常巡视现象");
            return;
        }
        if (aboutPatrols != null) {
            aboutPatrols.add(patrolDetailBean);
    ...
    private void doAllotSeat() {
        if (LocalSetting.AutoAllotSeat) {
            ...
            RestClient.get(url, new BaseAsyncHttpResponseHandler() {
                ...
                public void onSuccess(int i, String s) {
                    ...
                    if(seatMessage.contains("none")){
                        UIHelper.showToast(NewHandlerActivity.this,"没有剩余座位,请稍后重试!");
                    } else if(seatMessage.contains("has")){
                        UIHelper.showToast(NewHandlerActivity.this,"该座位已经被占用,请重新分配");
                    }else {
                        /**
                         * 更新数据库
                         */
                        ...
                        InfoUtils.bluetoothPrintSeatNo(NewHandlerActivity.this,seatMessage,PeopleContent);
                ...
                public void onFailure(int i, Throwable throwable, String s) {
                    dismissLoadingDialog();
                    realeaseAllButtons();
                    UIHelper.showToast(NewHandlerActivity.this,"自动分配座位失败");
                }
            });
        } else {
            //            /**
            ...
            //             */
            //            doShowEmptySeats();
            startActivityForResult(new Intent(NewHandlerActivity.this,AreaInfusionEmptyActivity.class), 100);
        }
    }
    /**
     * 结束本次输液
    ...
    private void doFinish() {   //先判断有多少未输液
        ...
        RestClient.get(url, new BaseAsyncHttpResponseHandler() {
            ...
            @Override
            public void onFailure(int i, Throwable throwable, String s) {
                UIHepler.showToast(NewHandlerActivity.this, "判断几组未输失败");
            }
        });
    }
        ...
        RestClient.put(NewHandlerActivity.this, url, Json, new BaseAsyncHttpResponseHandler() {
            ...
            public void onSuccess(int i, String s) {
                ...
                if (strings != null && strings.size() > 0) {   //释放座位， 不管
                    ...
                    UIHepler.showToast(NewHandlerActivity.this, "结束本次输液成功");
                    if (unBottlesCount > 0) {
                        AlertDialog.Builder dialog = new AlertDialog.Builder(NewHandlerActivity.this);
            ...
            public void onFailure(int i, Throwable throwable, String s) {
                ...
                UIHepler.showToast(NewHandlerActivity.this, "结束本次输液失败");
            }
        });
    ...
    private void handlerGameOver() {
        ...
        RestClient.get(url, new BaseAsyncHttpResponseHandler() {
            ...
            public void onSuccess(int i, String s) {
                ...
                InfoUtils.sendSuccessBroadcast(context);
                UIHepler.showToast(NewHandlerActivity.this, "结束全部输液成功");
                finish();
            }
            ...
            public void onFailure(int i, Throwable throwable, String s) {
                dismissLoadingDialog();
                realeaseAllButtons();
                UIHepler.showToast(NewHandlerActivity.this, "结束所有输液失败");
            }
        });
    }
        ...
        RestClient.get(url, new BaseAsyncHttpResponseHandler() {
            ...
            public void onSuccess(int i, String s) {
                ...
                if ("none".equals(seatMessage)) {
                    UIHepler.showDilalog(NewHandlerActivity.this,"没有剩余座位,请稍后重试!");
                } else if ("has".equals(seatMessage)) {
                    UIHepler.showDilalog(NewHandlerActivity.this,emptySeatNO1 + "座位被占用，请选择其他座位！");
                } else {
                    ...
                    if(isUseSeatNo){
                        if(printModeString.contains("蓝牙打印")){
                            String PeopleContent=currentBottle.PeopleInfo.Name +":" +currentBottle.PeopleInfo.PatId ;
                            InfoUtils.bluetoothPrintSeatNo(NewHandlerActivity.this,StringUtils.getString(divZone) + ":" + currentBottle.PeopleInfo.SeatNo,PeopleContent);
                        }
                    }
